...
interface Column {
  id: 'username' | 'userType' | 'userStatus' | 'actions';
  label: string;
  minWidth?: number;
  align?: 'right';
}
    ...
    <Paper sx={{ width: '100%', overflow: 'hidden' }}>
      <TableContainer sx={{ maxHeight: 440 }}>
        <Table stickyHeader aria-label="sticky table">
          <TableHead>
            <TableRow>
              <TableCell align="left" style={{ minWidth: 50 }}>Count</TableCell> {/* Add Count Column */}
              {columns.map((column) => (
                <TableCell
                  key={column.id}
                  align={column.align}
                        ...
                        return (
                          <TableCell key={column.id} align={column.align}>
                            <div>
                              <Button
                                variant="outlined"  
                                color="secondary" // Suspend button
      ...
      <TablePagination
        ...
        component="div"
        count={rows.length} // Total count of rows
